...
/* Bell popover: notifications stay on the current page */
.st-key-carermate_bell {
  position:fixed;
  top:1.0rem;
  right:max(12px, calc((100vw - 760px)/2 + 10px));
  z-index:10020;
  width:auto!important;
}
.st-key-carermate_bell button {
  width:auto!important;
...
def apply_theme():
    ...
    if theme == "light":
        ...
        values = """
          --bg:#0f1115; --panel:#171a20; --panel2:#1d2128; --text:#f7f4ee;
          --muted:#a7abb4; --accent:#22c55e; --accent2:#86efac; --green:#22c55e;
          --orange:#f59e0b; --red:#ef4444; --border:#2a2f38;
          --accent-soft:#10251a; --accent-border:#245337; --accent-text:#86efac;
          --hero-bg:linear-gradient(145deg,#10251a,#171a20 58%);
          --progress-bg:#282c33; --nav-bg:rgba(15,17,21,.96); --nav-muted:#858a94;
        """
        scheme = "dark"
...
def render_notification_bell(today_):
    ...
    with st.container(key="carermate_bell"):
        with st.popover(bell_label, help="Notifications"):
            ...
            with st.expander("Reminder settings"):
                prefs = {
                    ...
                    "holiday_start": "Holiday — 1 day before",
                ...
                for key, label in prefs.items():
                    new_val = st.checkbox(
                        label,
                        value=D["notif_settings"].get(key, True),
                        key=f"bellpref_{key}",
                    )
                    if new_val != D["notif_settings"].get(key, True):
                        D["notif_settings"][key] = new_val
                        changed = True
                if changed:
                    save_data(D)
...
render_notification_bell(today)
...
if page == "Home":
    ...
    # Home intentionally stops after the six summary cards.
