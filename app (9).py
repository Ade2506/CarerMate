...
def make_carermate_icon(size=128):
    """White app icon with CarerMate green/red initials."""
    ...
    # Center a compact CM mark; green C, red M.
    c, m = "C", "M"
    cb = draw.textbbox((0, 0), c, font=font)
    mb = draw.textbbox((0, 0), m, font=font)
    cw, mw = cb[2]-cb[0], mb[2]-mb[0]
    total = cw + mw - int(size*0.05)
    x = (size-total)//2
    y = int(size*0.20)
    draw.text((x, y), c, font=font, fill="#138A43")
    draw.text((x+cw-int(size*0.05), y), m, font=font, fill="#D9272E")
    # Small two-colour underline inspired by the employer palette, not a copy of its logo.
    pad = int(size*0.20)
    mid = size//2
    uy = int(size*0.78)
    draw.rounded_rectangle((pad, uy, mid, uy+int(size*0.045)), radius=3, fill="#138A43")
    draw.rounded_rectangle((mid, uy, size-pad, uy+int(size*0.045)), radius=3, fill="#D9272E")
    return img

APP_ICON = make_carermate_icon()
...
def apply_theme():
    ...
    if theme == "light":
        st.markdown(
            ...
            .bottom-nav { background:rgba(247,248,247,.97)!important; }
            .hero { background:linear-gradient(145deg,#eef9f1,#ffffff 60%)!important; }
            .brand-pill, .bottom-nav a.active { background:#e8f6ed!important; }
            .stButton button, .stFormSubmitButton button { background:#eaf7ee!important; color:#087538!important; }
            </style>
            """,
            unsafe_allow_html=True,
        )
...
def render_brand(page):
    st.markdown(
        f"<div class='brand'><div class='brand-name'><span class='brand-logo'><span class='c'>C</span><span class='m'>M</span></span><span>CarerMate</span></div><div class='brand-pill'>{page}</div></div>",
        unsafe_allow_html=True,
    )
