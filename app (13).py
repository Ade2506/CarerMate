...
# iPhone/iPad Home Screen icon metadata.
# The cache-busting query string helps iOS notice icon updates.
components.html(
    """
    <script>
    (function () {
      ...
      upsertLink('apple-touch-icon', '/app/static/apple-touch-icon.png?v=5');
      upsertLink('icon', '/app/static/apple-touch-icon.png?v=5');
...
:root {
  ...
  --border:#304461;
  --accent-soft:#42263b;
  --accent-border:#a64c68;
  --accent-text:#ffe0e5;
  --hero-bg:radial-gradient(circle at 95% 5%,rgba(241,87,113,.38),transparent 34%),linear-gradient(130deg,#183c79,#0c234c 65%,#11213c);
  --progress-bg:#263953;
  --nav-bg:rgba(9,20,41,.97);
  --nav-muted:#a2b2cf;
}
...
.brand-logo .c { color:#2459ac; } .brand-logo .m { color:#d53857; }
...
.hero { background:var(--hero-bg); border:1px solid rgba(136,178,255,.35); border-radius:24px; padding:24px; box-shadow:0 20px 45px rgba(0,11,39,.16); margin:8px 0 14px; }
.hero-label { color:#d4e4ff; font-size:.78rem; text-transform:uppercase; letter-spacing:.08em; }
.hero-value { color:#fff; font-size:2.35rem; line-height:1.05; font-weight:850; margin-top:6px; }
.hero-sub { color:#d7e6ff; margin-top:6px; font-size:.9rem; }
...
.card-green { background:linear-gradient(145deg,rgba(54,115,221,.28),var(--panel) 75%); border-color:rgba(91,157,255,.46); }
...
.card-purple { background:linear-gradient(145deg,rgba(91,157,255,.20),var(--panel) 68%); border-color:rgba(91,157,255,.35); }
.card-amber { background:linear-gradient(145deg,rgba(245,158,11,.18),var(--panel) 68%); border-color:rgba(245,158,11,.42); }
.card-teal { background:linear-gradient(145deg,rgba(91,157,255,.17),var(--panel) 68%); border-color:rgba(91,157,255,.38); }
...
.progress-fill { height:100%; background:linear-gradient(90deg,#2868d2,#ef5970); border-radius:99px; }
...
def apply_theme():
    ...
    if theme == "light":
        values = """
          --bg:#f3f6fc; --panel:#ffffff; --panel2:#eaf0fa; --text:#14294d;
          --muted:#536787; --accent:#c7294b; --accent2:#a51e3e; --green:#255fbd;
          --orange:#a86409; --red:#c7294b; --border:#d7e1f0;
          --accent-soft:#ffe7ec; --accent-border:#f3b7c4; --accent-text:#9f1837;
          --hero-bg:radial-gradient(circle at 95% 5%,rgba(255,118,140,.42),transparent 32%),linear-gradient(130deg,#285cc0,#143b82 68%,#102d69);
          --progress-bg:#dce5f4; --nav-bg:rgba(249,251,255,.97); --nav-muted:#617393;
        """
        scheme = "light"
    else:
        values = """
          --bg:#091429; --panel:#11213c; --panel2:#1a2c4a; --text:#f6f9ff;
          --muted:#aebfda; --accent:#e84b62; --accent2:#ffabb8; --green:#5b9dff;
          --orange:#f7bb69; --red:#f36a7d; --border:#304461;
          --accent-soft:#42263b; --accent-border:#a64c68; --accent-text:#ffe0e5;
          --hero-bg:radial-gradient(circle at 95% 5%,rgba(241,87,113,.38),transparent 34%),linear-gradient(130deg,#183c79,#0c234c 65%,#11213c);
          --progress-bg:#263953; --nav-bg:rgba(9,20,41,.97); --nav-muted:#a2b2cf;
        """
        scheme = "dark"
    ...
    st.markdown(
        ...
        [data-testid="stExpander"], .card, .status-row, .notification-card, .cal-day {{
          color:var(--text)!important;
          border-color:var(--border)!important;
        }}
        .bottom-nav {{
...
def render_bottom_nav(active):
    pages = ["Home", "Calendar", "Leave", "Training", "Pay", "Profile"]
    labels = {
        "Home": "🏠\nHome",
        "Calendar": "📅\nCalendar",
        "Leave": "🌴\nLeave",
    ...
    with st.container(key="bottom_nav_native"):
        chosen = st.radio(
            "Navigation",
            options=[labels[p] for p in pages],
    ...
    return next(p for p in pages if labels[p] == chosen)
...
def render_brand(page, notif_count=0):
    ...
    with right:
        ...
        st.markdown(f"<div class='brand-pill header-page-pill'>{page}</div>", unsafe_allow_html=True)
...
def render_notification_bell(today_):
    ...
    with st.popover(bell_label, help="Notifications", use_container_width=True):
...
if page == "Home":
    ...
    st.markdown(
        ...
        + card("Est. take-home", f"£{tax_ni['net']:.2f}", f"Gross £{period_gross:.2f}", "💷", "blue")
        + "</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div class='grid2'>"
        + card("Next payday", fmt_short(payday.isoformat()), f"{max(0,(payday-today).days)} days away", "💳", "blue")
    ...
    for col, label, destination in zip(action_cols, ["📅 Calendar", "🌴 Leave", "💷 Pay"], ["Calendar", "Leave", "Pay"]):
        with col:
            if st.button(label, key=f"home_{destination}", use_container_width=True):
