...
def apply_theme():
    ...
    if theme == "light":
        st.markdown(
            ...
            :root {
              color-scheme: light;
              --bg:#f7f8f7; --panel:#ffffff; --panel2:#f0f3f1; --text:#151816;
              --muted:#69716c; --accent:#138A43; --accent2:#087538; --green:#138A43;
              --orange:#d97706; --red:#D9272E; --border:#d9dedb;
              --accent-soft:#e8f6ed; --accent-border:#b9ddc5; --accent-text:#087538;
              --hero-bg:linear-gradient(145deg,#eef9f1,#ffffff 60%);
              --progress-bg:#e3e8e5; --nav-bg:rgba(247,248,247,.97); --nav-muted:#69716c;
            }
            ...
            .bottom-nav, .card, .status-row, [data-testid="stExpander"], .cal-day {
...
def render_brand(page, notif_count=0):
    badge_html = f"<span class='notify-badge'>{notif_count}</span>" if notif_count else ""
    bell = f"<a class='notify-bell' href='?page=Notifications' aria-label='Notifications'>🔔{badge_html}</a>"
    pill = "" if page == "Notifications" else f"<div class='brand-pill'>{page}</div>"
    st.markdown(
        f"<div class='brand'><div class='brand-name'><span class='brand-logo'><span class='c'>C</span><span class='m'>M</span></span><span>CarerMate</span></div><div class='brand-actions'>{bell}{pill}</div></div>",
        unsafe_allow_html=True,
    )
...
# NAVIGATION
# -----------------------------------------------------------------------------
allowed_pages = ["Home", "Calendar", "Leave", "Training", "Pay", "Profile", "Notifications"]
...
if page == "Home":
    ...
    st.markdown(
        ...
        "<div class='quick-chip'><span class='qicon'>📅</span>Rota</div>"
        "<div class='quick-chip'><span class='qicon'>🎓</span>Training</div>"
        "<div class='quick-chip'><span class='qicon'>💚</span>CarerMate</div>"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    if current_alerts:
        ...
        first = current_alerts[0]
        st.markdown(
            f"<a href='?page=Notifications' style='text-decoration:none!important'><div class='notification-card'><div class='notification-icon'>{first['icon']}</div><div><div class='notification-title'>{first['title']}</div><div class='notification-body'>{first['body']}</div><div class='notification-time'>View all notifications →</div></div></div></a>",
            unsafe_allow_html=True,
        )
...
# -----------------------------------------------------------------------------
    ...
    st.caption("Shift, holiday, training and payday reminders in one place.")

    alerts = notification_items(today, include_upcoming=False)
    upcoming = notification_items(today, include_upcoming=True)
    ...
    if alerts:
        ...
        st.success("You’re all caught up — no reminders need your attention right now.")

    if upcoming_only:
        st.markdown("<div class='section-title'>Coming up</div>", unsafe_allow_html=True)
        for item in upcoming_only[:12]:
            st.markdown(
                f"<div class='notification-card'><div class='notification-icon'>{item['icon']}</div><div><div class='notification-title'>{item['title']}</div><div class='notification-body'>{item['body']}</div><div class='notification-time'>{item['when']}</div></div></div>",
                unsafe_allow_html=True,
            )
    ...
    with st.expander("Reminder settings"):
        st.write("Choose which reminders CarerMate should show.")
        prefs = {
            "shift_tonight": "Shift-day hydration reminder",
            "shift_tomorrow": "Shift tomorrow",
            "holiday_5_days": "Holiday reminder — 5 days before",
            "holiday_3_days": "Holiday reminder — 3 days before",
            "holiday_start": "Holiday reminder — 1 day before",
            "holiday_pending": "Pending holiday request",
            "training_5_days": "Training reminder — 5 days before",
            "training_3_days": "Training reminder — 3 days before",
            "training_overdue": "Training overdue",
            "pay_period_end": "Pay period ending",
            "payday_tomorrow": "Payday tomorrow",
        }
        for key, label in prefs.items():
            D["notif_settings"][key] = st.checkbox(label, value=D["notif_settings"].get(key, True), key=f"bellpref_{key}")
        if st.button("Save reminder settings", key="save_bell_prefs"):
            save_data(D)
...
# -----------------------------------------------------------------------------
    ...
    with st.expander("Appearance"):
        ...
        if st.button("Apply theme", key="apply_theme_btn"):
            D.setdefault("appearance", {})["theme"] = theme_choice.lower()
            save_data(D)
            st.rerun()
