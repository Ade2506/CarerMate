...
@media (max-width: 520px) {
  .st-key-bottom_nav_native [role="radiogroup"] label p { font-size:.62rem !important; }
  .st-key-bottom_nav_native [role="radiogroup"] label { padding:7px 0 !important; }
}
</style>
""",
    unsafe_allow_html=True,
...
allowed_pages = main_pages + ["Notifications"]
...
# Notifications is still opened from the bell. Main bottom tabs switch without a
# browser refresh. Tapping any bottom tab leaves Notifications immediately.
page = "Notifications" if requested_page == "Notifications" else selected_main_page
