...
if page == "Home":
    ...
    if p_start:
        ...
        st.warning("Set the employer's actual 4-week pay-period start date in the Pay tab before relying on pay or payday figures.")

    st.subheader("Payday")
    if payday:
        st.write(f"Next payday: **{fmt_short(payday.isoformat())}** ({(payday - today).days} days away)")
    else:
        st.write("Payday will appear after the 4-week pay cycle is configured.")
    ...
    if next_train:
        st.write(f"Next due: **{next_train['name']}** — {badge(training_status(next_train))} — "
                  f"{training_due_label(next_train)}")
